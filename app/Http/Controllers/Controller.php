...
class Controller extends BaseController
{
    ...
    public function calculate_orders_min_discounts(Request $request, $data = array())
    {
        ...
        if ( count($products_info) > 0 ) {
            foreach ($products_info as $key => $value) {       
                ...
                $min_disc = $value['min_discount'];
                $max_disc = $value['max_discount'];
                $min_qty = $value['min_qty'];
                $max_qty = $value['max_qty'];

                $min_discount = ( ($consume_qty - $min_qty) / ($max_qty - $min_qty) ) * ( $max_disc - $min_disc ) + $min_disc;

                $after_discount = round( ((100-$min_discount)/100) * $before_disc_earning,2);
     ...
     * @return \Illuminate\Http\Response
     */
    public function calculate_orders_min_discounts__TEMP(Request $request, $data = array())
    {
        $posted_data = array();
        $posted_data['min_disc_qualify'] = true; // means bulk products
        $posted_data['within_time_limit'] = date("Y-m-d");
     ...
     * @return \Illuminate\Http\Response
     */
    public function calculate_orders_max_discounts__TEMP(Request $request, $posted_data = array())
    {
        $posted_data = array();
        $posted_data['max_disc_qualify'] = true; // means bulk products
        $posted_data['within_time_limit'] = date("Y-m-d");
